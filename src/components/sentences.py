...
def get_open_colab_sentence(page:TypedPage, settings_name: str, input_zip_file: str):
    ...
    async def create_colab_url(e):
        ...
        if settings_name == "" or input_zip_file == "":
            set_url_status("欲しいスキル設定か画像zipファイルの名前が設定されていません", "red")
            return

        qc_settings = await qc_settings_repo.fetch(user_id=user_id, settings_name=settings_name)
        ...
        if target_cell:
            ...
            for var_name, new_value in updata_vars.items():
                ...
                target_cell.source = re.sub(pattern, replacement, target_cell.source, flags=re.MULTILINE)

        gist_filename = "temp_modified"
        try:
            res_data = await page.app_state.shared_gist.upload_ipynb(content=nbformat.writes(nb), file_name=gist_filename)
            gist_id = res_data["id"]
            gist_username = res_data["owner"]["login"]
        except httpx.HTTPStatusError as ex:
            set_url_status(f"Gistへのアップロードに失敗しました: {ex.response.status_code} {ex.response.text}", "red")
            return
        else:
