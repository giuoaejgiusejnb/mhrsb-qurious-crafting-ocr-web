...
from constants import COL_DESIRED_SKILLS_SETTINGS, COL_USERS, FIELD_SKILLS
...
class QCSettingsRepository(BaseRepository[QCSettings]):
    ...
    async def fetch(self, user_id: str, settings_name: str) -> QCSettings:
        return await self.get(
            collection_path=self.get_collection_path(user_id), document_id=settings_name
        )
    ...
    async def delete_settings(
        self, user_id: str, settings_name: str, batch: AsyncWriteBatch | None = None
    ) -> None:
